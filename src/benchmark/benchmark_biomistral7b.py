...
Benchmarks the BioMistral 7B model for the radiotherapy summarization task.
"""
...
if __name__ == "__main__":
    base_dir = "saved_files/Benchmark_BIOMISTRAL7B"
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        ...
        for i in range(X):
            ...
            prompt_filename = os.path.join(prompt_dir, f"prompt_BIOM7B_{i}.txt")
            with open(prompt_filename, "w", encoding="utf-8") as f:
                f.write(prompt)
                f.write(str(scores))

            output_filename = os.path.join(output_dir, f"output_BIOM7B_{i}.txt")
