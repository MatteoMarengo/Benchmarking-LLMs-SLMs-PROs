...
Benchmarks the Meta Llama-2-7b-chat-hf model for the radiotherapy summarization task.
"""
...
def main(prompt, scores, QA, answers, prompt_filename, output_filename, csv_writer, prompt_number):
    ...
    system_prompt = ""
    # Llama 2 chat models use: <s>[INST] <<SYS>>{system_prompt}<</SYS>> {user_prompt} [/INST]
    prompt_with_tags = f"<s>[INST] <<SYS>>\n{system_prompt}\n<</SYS>>\n\n{prompt} [/INST]"
...
if __name__ == "__main__":
    base_dir = "saved_files/Benchmark_LLAMA27B"
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        ...
        for i in range(X):
            ...
            prompt_filename = os.path.join(prompt_dir, f"prompt_Llama2_7B_{i}.txt")
            with open(prompt_filename, "w", encoding="utf-8") as f:
                f.write(prompt)
                f.write(str(scores))

            output_filename = os.path.join(output_dir, f"output_Llama2_7B_{i}.txt")
